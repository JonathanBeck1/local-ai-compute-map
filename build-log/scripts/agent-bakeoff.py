...
ROOT = os.environ.get("BAKEOFF_ROOT", os.path.join(HOME, "repos", "agent-trial"))  # holds TASK.md
TASK = open(os.path.join(ROOT, "TASK.md")).read()
EVID = os.environ.get("BAKEOFF_RESULTS", os.path.join(os.getcwd(), "bakeoff-results"))
...
NOISE = ("/.claude", "/.cache", "/.local/", "/.config", "/snap/", "/bakeoff-results",
...
def main():
    ...
    proj = os.path.join(ROOT, "bakeoff", safe)
