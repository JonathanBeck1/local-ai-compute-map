...
Phase 00 is up: no Time Machine destination configured at all, and 28 repos with uncommitted work against 7 with unpushed commits. Phase 02 is up: the gaming PC is now an Ubuntu CUDA node, and Docker 29 put the first image on the wrong disk while its own check said otherwise. Phase 03 is up: Ollama on that box, four models with measured tok/s, and a 30B MoE that doesn't fit the 12 GB card running as fast as a 12B that does. The rest go up as I run them.
